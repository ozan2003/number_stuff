...
mod tests
{
    ...
    fn test_determine_k()
    {
        ...
        assert_eq!(determine_k(&Integer::parse("999999").unwrap().complete()), 10);
        assert_eq!(determine_k(&Integer::parse("1000000").unwrap().complete()), 20); // < 10^9
        assert_eq!(determine_k(&Integer::parse("999999999").unwrap().complete()), 20);
        assert_eq!(determine_k(&Integer::parse("1000000000").unwrap().complete()), 50); // >= 10^9
        assert_eq!(determine_k(&Integer::parse("999999999999").unwrap().complete()), 50);
        assert_eq!(determine_k(&Integer::parse("1000000000000").unwrap().complete()), 50);
    }
}